...
placeholder_adj_mat = tf.placeholder(tf.float32, [None, ])
...
network = GcnNet()
logits = network.buildNet(placeholder_node_count, placeholder_adj_mat)
cost = tf.reduce_mean(
...
with tf.Session() as sess:
	...
	for epoch in range(epoches):
		for feature, label in zip(features, labels):
			...
			feed = {placeholder_learning_rate: learning_rate, placeholder_adj_mat: adj_mat, 
